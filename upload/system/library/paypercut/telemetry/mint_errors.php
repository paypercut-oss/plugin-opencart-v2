 ...
 *
 * Turns an HTTP status (plus whatever the body offers) into a reason code, a
 * message, and whether clicking Start again could plausibly work.
 */
class PaypercutMintErrorMapper
{
    ...
    /**
     * Map a mint response onto { reason_code, message, retryable }.
     *
     * Branches on the HTTP status first and consults the body only to refine
     * copy: the mint surfaces its gates as bare gRPC statuses with no
     * public-error metadata, so `telemetry_token_key_inactive` and friends
     * arrive capitalised in `message` with no `code` key at all - hence
     * substring matching, not code equality.
     *
     * @param int   $status HTTP status, or 0 for a transport failure.
     * @param array $body   Decoded response body, empty when absent.
     */
    public static function map($status, $body = array())
    {
        $status = (int)$status;
        ...
        switch ($status) {
            ...
            case 400:
                if (strpos($detail, 'ineligible') !== false) {
                    return self::result(
                        'key_ineligible',
                        "This store's Paypercut API key isn't eligible for debug sessions yet - this usually means the key isn't fully activated on your Paypercut account. Nothing has been sent. Contact Paypercut support and quote your account name from the API Configuration section above.",
                        false
                    );
                }
     ...
     *
     * @param int $skew_seconds Signed difference between the mint's clock and this server's.
     */
    public static function clockSkew($skew_seconds)
    {
        $minutes = (int)round(abs((int)$skew_seconds) / 60);
     ...
     */
    private static function detail($body)
    {
        $parts = array();
